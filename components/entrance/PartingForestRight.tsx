...
export default function PartingForestRight() {
  return (
    <motion.div
      initial={{ x: 0 }}
      animate={{ x: "100%" }}
      ...
      className="absolute top-0 right-0 bottom-0 w-1/2 overflow-hidden z-30"
      style={{
        background: "linear-gradient(to left, #0a1a0a 0%, #1a2e1a 100%)",
      }}
    >
      ...
      <div className="absolute inset-0 opacity-40">
        {[...Array(8)].map((_, i) => (
          ...
          >
            {/* Fern frond - mirrored */}
            <svg viewBox="0 0 100 120" className="w-full h-full" style={{ filter: "blur(2px)", transform: "scaleX(-1)" }}>
              <path
      ...
      <div className="absolute inset-0 opacity-70">
        {[...Array(12)].map((_, i) => (
          <motion.div
            ...
            className="absolute"
            style={{
              right: `${10 + Math.random() * 90}%`,
              top: `${Math.random() * 100}%`,
              width: `${50 + Math.random() * 40}px`,
              height: `${60 + Math.random() * 50}px`,
            }}
          >
            {/* Botanical leaf - mirrored */}
            <svg viewBox="0 0 60 80" className="w-full h-full" style={{ transform: "scaleX(-1)" }}>
              <path
      ...
      <div
        ...
        style={{
          background: "linear-gradient(to right, rgba(26, 46, 26, 0) 0%, rgba(26, 46, 26, 0.8) 100%)",
        }}
      />
    </motion.div>
  );
